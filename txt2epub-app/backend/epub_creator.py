...
def create_epub(output_path, title, author, chapters, cover_path=None, lang='zh-CN', book_id=None):
    """
    ...
    def create_chapter_html(title: str, content: str, level: int = 1) -> str:
        ...
        lines = [f'<p>{line}</p>' for line in content.split('\n') if line.strip()]
    ...
    if is_hierarchical(chapters):
        ...
        for i, (t, c) in enumerate(chapters):
            lines = [f'<p>{line}</p>' for line in c.split('\n') if line.strip()]
            html_content = f'<h1>{t}</h1>\n' + '\n'.join(lines)
