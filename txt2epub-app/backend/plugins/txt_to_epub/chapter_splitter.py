...
# Maximum suggested level constant
MAX_LEVEL = 99
    ...
    def split_hierarchical(self, text: str, patterns: List[str], levels: List[int] = None) -> List[Dict[str, Any]]:
        ...
        def split_level(content: str, pattern_idx: int) -> List[Dict[str, Any]]:
            ...
            for i, match in enumerate(matches):
                ...
                # If children exist, calculate remaining content before first child
                if children:
                    # Use regex to find the first child's title position accurately
                    first_child_title = children[0]["title"]
                    # Escape special regex characters in the title for safe matching
    ...
    def scan(self, text: str) -> List[Dict]:
        ...
        for preset in self.PRESET_PATTERNS:
            try:
                ...
                # Use finditer for consistent handling of groups
