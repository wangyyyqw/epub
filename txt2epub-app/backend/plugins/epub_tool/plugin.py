...
class EpubToolPlugin(BasePlugin):
    ...
    def register_arguments(self, parser: argparse.ArgumentParser):
        parser.add_argument("--operation", choices=[
            "encrypt", "decrypt", "reformat", "s2t", "t2s", 
            "font_subset", "img_compress", "img_to_webp", 
            "webp_to_img", "phonetic", "pinyin", "footnote", "yuewei"
    ...
    def run(self, args: argparse.Namespace):
        ...
        try:
            if args.operation == "encrypt":
                result = encrypt_epub.run(args.input_path, args.font_path)
            ...
            elif args.operation == "pinyin":
                res = pinyin_annotate.run_add_pinyin(args.input_path)
                result = res[0] if isinstance(res, (tuple, list)) else res
